...
export function WeighIn({
  ...
  const bestValueIdx =
    ppgs.filter((p) => p !== Infinity).length > 1
      ? ppgs.findIndex(
          (p) =>
            p === minPpg &&
            ppgs.filter((pp) => Math.abs(pp - minPpg) <= 2).length <
              ppgs.filter((p2) => p2 !== Infinity).length,
        )
      : -1;
  ...
  const heaviestIdx =
    weights.filter((w) => w === maxW).length === 1 ? weights.indexOf(maxW) : -1;

  // Most pure gold
  const pureGolds = chains.map((c) => c.pureGold || 0);
  const maxPure = Math.max(...pureGolds);
  const mostGoldIdx =
    pureGolds.filter((p) => p === maxPure).length === 1
      ? pureGolds.indexOf(maxPure)
      : -1;
  ...
  const cheapestIdx =
    prices.filter((p) => p === minPrice).length === 1
  ...
  const purestIdx =
    karats.filter((k) => k === maxKarat).length === 1
      ? karats.indexOf(maxKarat)
      : -1;
  ...
  const rows: Row[] = [
    ...
    },
    {
      ...
      render: (c) => (
        <span>
          {c.pricePerPureGram ? `$${c.pricePerPureGram.toFixed(0)}/g` : '—'}
        </span>
      ),
      winIdx: bestValueIdx,
      winTag: 'better value',
    ...
    },
    {
      ...
      render: (c) => (
        <KaratStamp karat={c.karat} purity={KARAT_PURITY[c.karat] ?? 10 / 24} />
      ),
      winIdx: purestIdx,
      winTag: 'purer',
      alwaysShow: true,
    },
    {
      ...
      render: (c) => (
        <WeightBar
          weight={c.weight || 0}
          max={maxWeight}
          purity={KARAT_PURITY[c.karat] ?? 10 / 24}
        />
      ),
      winIdx: heaviestIdx,
      winTag: 'heavier',
    ...
    },
    {
      num: ROMAN[5],
      label: 'Melt Value',
      sub: 'gold content at spot price',
      render: (c) => <span>{c.meltValue ? formatUSD(c.meltValue) : '—'}</span>,
      winIdx: -1,
      winTag: '',
      alwaysShow: true,
    },
    {
      num: ROMAN[6],
      label: 'Value %',
      sub: 'how much of the price is gold',
      render: (c) => (
        <span>{c.valueScore ? `${c.valueScore.toFixed(0)}%` : '—'}</span>
      ),
      winIdx: (() => {
        const scores = chains.map((ch) => ch.valueScore || 0);
        const mx = Math.max(...scores);
        return scores.filter((s) => s === mx).length === 1
          ? scores.indexOf(mx)
          : -1;
      })(),
      winTag: 'best',
    ...
    },
    {
      num: ROMAN[7],
      label: 'Weight/Inch',
      sub: 'grams per inch of chain',
      render: (c) => (
        <span>{c.weightPerInch ? `${c.weightPerInch.toFixed(3)}g` : '—'}</span>
      ),
      winIdx: -1,
      winTag: '',
    },
    {
      num: ROMAN[8],
      label: 'Gold/Inch',
      sub: 'pure gold per inch',
      render: (c) => (
        <span>{c.goldPerInch ? `${c.goldPerInch.toFixed(3)}g` : '—'}</span>
      ),
      winIdx: -1,
      winTag: '',
    },
  ...
  return (
    <div className="weighin-root">
      ...
      <div className="weighin-header">
        <div>
          ...
          <h2 className="weighin-title">
            {count === 2 ? 'Two' : count === 3 ? 'Three' : 'Four'} chain
            {count !== 1 ? 's' : ''},<br />
            <span
              style={{
      ...
      {/* ─── CHAIN COLUMNS ─── */}
      <div
        className="weighin-columns"
        style={{gridTemplateColumns: chains.map(() => '1fr').join(' ')}}
      >
        {chains.map((chain, idx) => (
          <ChainColumn
            ...
            onRemove={
              chain.type === 'custom' && onRemoveCustom
                ? () => {
                    const customIdx =
                      chains
                        .slice(0, idx + 1)
                        .filter((c) => c.type === 'custom').length - 1;
      ...
      <div className="weighin-ledger">
        {/* Header */}
        <div
          className="weighin-ledger-header"
          style={{gridTemplateColumns: ledgerCols}}
        >
          <div>№</div>
          <div>Measure</div>
          {chains.map((c, i) => (
            <div key={i} className={`weighin-ledger-header-${i}`}>
              {shortNames[i]}
            </div>
          ))}
        </div>
        ...
        {activeRows.map((row, ri) => (
          <div
            key={row.label}
            className="weighin-row"
            style={{
              gridTemplateColumns: ledgerCols,
              borderBottom:
                ri === activeRows.length - 1
                  ? 'none'
                  : `1px solid ${STYX.line}`,
            }}
          >
      ...
      <div className="weighin-footnote">
        <span>Spot {formatUSD(spotPerOz)}/oz</span>
...
function VerdictText({
  ...
  const bestPpgIdx = ppgs.indexOf(
    Math.min(...ppgs.filter((p) => p !== Infinity)),
  );

  const allSameWeight = new Set(weights).size === 1;
  const allSameKarat = new Set(chains.map((c) => c.karat)).size === 1;
  const allSamePpg = ppgs
    .filter((p) => p !== Infinity)
    .every((p) => Math.abs(p - ppgs[bestPpgIdx]) <= 2);
  ...
  return (
    <>
      {!allSameWeight ? (
        <>
          The{' '}
          <strong style={{fontWeight: 600}}>{shortNames[heaviestIdx]}</strong>{' '}
          is the heaviest at <NumInline>{weights[heaviestIdx]}g</NumInline> with{' '}
          <NumInline>{pureGolds[heaviestIdx].toFixed(1)}g</NumInline> of pure
          gold.{' '}
        </>
      ) : (
      ...
      {!allSamePpg && ppgs[bestPpgIdx] !== Infinity ? (
        <>
          The{' '}
          <strong style={{fontWeight: 600}}>{shortNames[bestPpgIdx]}</strong> is
          the best value at{' '}
      ...
      <div style={{position: 'relative', overflow: 'hidden'}}>
        {chain.type === 'product' && chain.image ? (
          <Link to={`/products/${chain.handle}`}>
            <Image
              data={chain.image}
              alt={chain.image?.altText ?? chain.title}
              aspectRatio="4/5"
...
function KaratStamp({karat, purity}: {karat: number; purity: number}) {
  return (
    <div style={{display: 'flex', alignItems: 'center', gap: 10}}>
      <span>
        {karat}
        <span style={{fontSize: '0.6em'}}>K</span>
      </span>
...
function WeightBar({
  ...
  return (
    <div className="weighin-bar-wrap">
      <span>
        {weight}
        <span style={{fontSize: '0.5em', color: STYX.silt}}>g</span>
      </span>
        ...
        >
          <div
            style={{
              ...
              left: 0,
              width: `${pct}%`,
              background: `repeating-linear-gradient(45deg, ${STYX.silt} 0 5px, ${STYX.silt2} 5px 6px)`,
            }}
          />
          <div
            style={{
  ...
  return (
    <div className="weighin-bar-wrap">
      ...
      <div className="weighin-bar-track" style={{flex: 1, maxWidth: 160}}>
        ...
        >
          {weight > 0
            ? `${((pure / weight) * 100).toFixed(1)}% of total mass`
            : '—'}
        </div>
      </div>
    </div>
